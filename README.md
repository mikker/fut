...
At 120 columns and wider, a full-height 24-column peer-workspace sidebar docks outside the current workspace, leaving at least 96 columns for its tabs and terminal content. It follows the current session's workspaces in resource order, marks the current checkout with `●`, dims closing workspaces with `×`, remembers the last focused terminal in each workspace, and can be placed on the left or right. At 119 columns and below it consumes no permanent space. `Ctrl-b w` activates the docked list or opens the same outer list as an edge drawer without resizing the PTY; use `↑`/`↓` or `j`/`k`, `Home`/`End`, `Enter`, and `Esc` or `q`. Switching resolves to an exact stable terminal and waits for daemon acknowledgement before accepting more input. The sidebar preserves the selected/current workspace through live rename, close, movement, and overflow updates, and the global navigator remains the cross-session surface.
