...
Inside the client, `Ctrl-b k` opens the command bar, `Ctrl-b c` creates and switches to a default shell tab, `Ctrl-b g` opens the global navigator, `Ctrl-b w` activates workspace navigation, `Ctrl-b d` detaches, and `Ctrl-b Ctrl-b` sends a literal `Ctrl-b`. In a multi-pane tab, `Ctrl-b l` or `Ctrl-b o` focuses the next pane, `Ctrl-b h` or `Ctrl-b ;` focuses the previous pane, and `Ctrl-b z` toggles pane zoom. Navigation wraps in resource order; these are previous/next operations rather than geometric left/right navigation. Focus changes are acknowledged by the daemon before later input is read.
...
`Ctrl-b k` opens a borderless command bar over the terminal area without changing pane geometry or PTY size. It indexes the existing typed client actions—global and workspace navigation, tab creation, pane focus, zoom, and detach—and shows their actual direct bindings from the same binding catalog used for dispatch. Search is case-insensitive and token-based across names, aliases, and bindings. Use normal text or paste to filter, `↑`/`↓`, `Ctrl-p`/`Ctrl-n`, `Tab`/`BackTab`, `Home`/`End`, or page keys to choose, `Enter` to run, `Ctrl-u` to clear, and `Esc` or `Ctrl-c` to close. Empty results remain editable, Unicode backspace is grapheme-safe, pasted control runs cannot dispatch actions, and tiny clients retain a clipped prompt and selected action. Direct bindings and command choices invoke the same client-local typed dispatcher; neither path constructs CLI commands.

Multi-pane tabs use a responsive focus-biased horizontal accordion in resource order. When all panes fit, the focused pane receives at least 24 content columns, every sibling receives at least 12, and remaining width is distributed with twice the weight on focus. A one-cell neutral rail marks every visible pane; the focused rail is bold, background rails are dim, terminal colors are left untouched, and only the focused cursor is shown. If those minimums do not fit, the client falls back to the focused pane at full size and restores the accordion when space returns. Explicit client-local zoom also gives the focused pane the full terminal area, resizes its PTY, keeps pane cycling available, and shows a bold `zoom` status in the tab bar; changing tabs or losing the final sibling clears it. The exclusive input and resize lease remains on the focused terminal only, so separate clients may focus or zoom different sibling terminals; background panes are observed read-only and clipped to their current grids until focused.

The next layout slice replaces the accordion as the default with a shared authored split tree, matching tmux's split model. `Ctrl-b |` will split the focused pane to the right and `Ctrl-b _` will split it downward, initially at an even ratio and inheriting the focused terminal's current directory. The accordion remains available as an alternate client-owned layout policy rather than being approximated by a pane-width setting; zoom remains an overlay on either policy. The same slice adds wrapping `Ctrl-b n`/`Ctrl-b p` tab navigation and makes the command bar cover every dispatchable client action other than its own opener, enforced in tests.

Multi-pane lifecycle is supported: pane and terminal navigation is exact; pane placement can move between tabs in one workspace without disturbing its terminal; exiting or closing one pane preserves its siblings and ancestors; and closing a whole tab closes all descendant panes and terminals. When the focused terminal exits, Fut walks backward through pane, tab, and workspace resource order inside that session, then forward if there was no predecessor. It never crosses into another session: exhausting the attached session detaches that client and closes the empty session, while other sessions and the daemon remain alive. Interactive clients receive revisioned resource snapshots and authoritative complete tab views. External pane creation, movement, closure, and process exit therefore update every affected client without reselecting: background membership changes preserve focus, while a moved focused pane follows its stable terminal into the destination tab. Two-phase close remains visible in the model: a background pane disappears when its close is requested, while a focused pane remains selected through its final snapshot and exit before transferring or detaching with its nonzero child status. The navigator, tab bar, and workspace sidebar refresh from the same resource stream. The responsive accordion, zoom, navigation chrome, and command bar are client-owned presentation, not yet a persisted split system. Authored direction and ratios, pane titles, mouse focus, pane naming, cross-workspace movement, richer input, project recipes, and agent activity remain planned.
