...
- Copy mode, selection, scrollback search, and clipboard integration. (Wheel scrollback is implemented with per-attachment viewport state.)
- Truecolor, undercurl, hyperlinks, focus events, focused-application click forwarding, and extended keyboard protocols. (Mode-aware bracketed paste, pane click focus, and mode-aware wheel forwarding are implemented.)
