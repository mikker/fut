...
- a live horizontal tab/status bar with top/bottom global configuration and responsive overflow;
...
The next product slices continue prioritizing visible navigation and action discovery so the existing resource behavior is practical to dogfood. The live tab/status bar is implemented; next comes a responsive peer-workspace sidebar, then a searchable command bar over typed client actions. Deeper pane geometry and terminal ergonomics follow those surfaces rather than preceding them.
...
1. finish Milestone 4's initial navigation chrome with a responsive peer-workspace sidebar; the tab/status bar is implemented;
