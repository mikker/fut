...
- live navigation chrome: a horizontal tab/status bar with top/bottom global configuration and a responsive left/right peer-workspace sidebar with a narrow drawer;
...
The next product slices continue prioritizing visible navigation and action discovery so the existing resource behavior is practical to dogfood. The live tab/status bar and responsive peer-workspace sidebar are implemented; next comes a searchable command bar over typed client actions. Deeper pane geometry and terminal ergonomics follow those surfaces rather than preceding them.
...
1. add a searchable command bar so typed client actions are discoverable before every action has a dedicated binding; the initial tab and workspace navigation chrome is implemented;
2. continue Milestone 4's pane zoom, authored split geometry, navigation, and daily-driver terminal behavior while finishing the remaining split semantics from Milestone 2;
