...
- a noun-first CLI with exact raw-ID and location-aware resource operations, attaching `open` with an explicit background mode, exact attach commands, and a navigator-first top-level attach;
...
- `Ctrl-b h/j/k/l` uses unzoomed logical client geometry for non-wrapping directional pane focus; `Ctrl-b o` and `Ctrl-b ;` retain canonical cycling. `Ctrl-b P/T/W` toggles acknowledged per-client pane, tab, and workspace history, `Ctrl-b Ctrl-s` toggles session history, and `Ctrl-b 1` through `9` plus `0` select exact one-based tab slots 1 through 10.
...
- The command palette includes every dispatchable `ClientAction` except `OpenCommandBar` itself. A reverse catalog test fails if any direct binding or action lacks a launcher definition or unique tmux-style command name. Direct bindings and palette choices continue through one typed dispatcher.
...
Fut loads versioned extension packages through a language-neutral process boundary rather than an in-process plugin runtime. An extension has a stable namespaced ID, strict compatibility and capability metadata, and a root from which direct argv such as `./bin/refresh` can resolve. It may contribute only capabilities that require Fut cooperation:
...
The daemon owns one immutable generation-stamped registry. Reload prepares and validates a complete candidate, synchronizes its catalog across attached clients, and swaps it atomically; queued lifecycle events retain the generation active when they committed. Users may still configure explicit local roots, or install bounded immutable copies into Fut's managed store. Managed packages start disabled, record content hashes and provenance, and may be fetched only from an explicit pinned Git commit. Fut performs no discovery, build scripts, dependency resolution, automatic updates, native UI, or in-process SDK loading. Extension executables remain explicitly trusted user code. A capability-sandboxed WebAssembly tier is deferred until concrete lower-trust demand justifies its binary, security, and maintenance cost.
...
The explicit catalog, daemonless approval commands, trusted one-time workspace recipe creation flow, trusted extension settings, optional worktree discovery, and managed-command auto-start are implemented. In-client project opening and approval, trust preview, and configuration explanation remain.
...
- Add a searchable command palette over typed client actions. Open it with `Ctrl-b :`, show each action's tmux-style command name and direct binding when one exists, and let direct keys and the palette invoke the same action rather than constructing CLI commands. Treat terminal delivery of macOS Command-modified keys as a later compatibility path, not the baseline binding. (Implemented with every dispatchable client action except its own opener, one catalog and dispatcher, reverse coverage invariants, filtering, bounded input, paste isolation, and modal behavior.)
