...
The client/daemon wire format is intentionally unversioned during rapid development: its reserved epoch remains `0`, and compatibility between different builds is not promised. Stop the running daemon before replacing a build. Stable protocol versioning will begin when the wire format settles. Global `--json` is available only for noninteractive control commands. Successful output has a versioned envelope and dotted command name:
...
With multiple workspaces, a full-height 24-column workspace sidebar docks at 120 columns and wider, leaving at least 96 columns for tabs and terminal content. It follows the current session's workspaces in resource order, marks the active workspace with a leading bullet and bold row, gives keyboard selection a muted background, shows each stable workspace root on a muted second line, dims closing workspaces with `×`, remembers the last focused terminal in each workspace, and can be placed on the left or right. It collapses by default when only one workspace is active, and at 119 columns and below it consumes no permanent space; both behaviors are configurable. `Ctrl-b w` activates the docked list or opens the same outer list as an edge drawer without resizing the PTY; use `↑`/`↓` or `j`/`k`, `Home`/`End`, `Enter`, and `Esc` or `q`. Press `c` to create and switch to a logical workspace rooted at the focused terminal's current directory, or `r` to rename the selected workspace. Switching and creation resolve to exact stable terminals and wait for daemon acknowledgement before accepting more input. The sidebar preserves the selected/current workspace through live rename, close, movement, creation, and overflow updates, and the global navigator remains the cross-session surface.
