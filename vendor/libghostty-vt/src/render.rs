...
use std::{convert::Into, ffi::c_void, marker::PhantomData, mem::MaybeUninit};

use crate::{
    alloc::{Allocator, Object},
    error::{Error, Result, from_optional_result, from_result},
    ffi,
    screen::{Cell, CellContentTag, Row},
...
impl CellIteration<'_, '_> {
    ...
    /// Read the common raw cell fields in two batched FFI calls.
    pub fn basic_data(&self) -> Result<CellBasicData> {
        let mut raw = ffi::Cell::default();
        let mut has_styling = false;
        let row_keys = [
            ffi::RenderStateRowCellsData::RAW,
            ffi::RenderStateRowCellsData::HAS_STYLING,
        ];
        let mut row_values = [
            std::ptr::from_mut(&mut raw).cast::<c_void>(),
            std::ptr::from_mut(&mut has_styling).cast::<c_void>(),
        ];
        let mut written = 0;
        let result = unsafe {
        ...
        let mut content_tag = ffi::CellContentTag::CODEPOINT;
        let mut codepoint = 0;
        let cell_keys = [ffi::CellData::CONTENT_TAG, ffi::CellData::CODEPOINT];
        let mut cell_values = [
            std::ptr::from_mut(&mut content_tag).cast::<c_void>(),
            std::ptr::from_mut(&mut codepoint).cast::<c_void>(),
        ];
        let result = unsafe {
        ...
        Ok(CellBasicData {
            raw_cell: Cell(raw),
            content_tag: content_tag.try_into().map_err(|_| Error::InvalidValue)?,
            codepoint,
            has_styling,
        })
    }
