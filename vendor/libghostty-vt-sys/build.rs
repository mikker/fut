...
fn build_vendored(link_mode: LinkMode) {
    ...
    build
        ...
        // Distributed Cargo artifacts must not inherit the build host's CPU
        // features. Opting into `native` or a named Zig CPU remains available
        // for local, machine-specific builds.
        .arg(format!("-Dcpu={cpu}"))
