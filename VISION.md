...
The core protocol exposes resources, mutations, snapshots, terminal updates, and typed events. It does not require every client to consume one server-rendered application screen. The initial Ratatui client may live in the same repository and executable, but the core model must not depend on Ratatui widgets or global UI focus. Until simultaneous pane layout is implemented, it renders only the selected pane at full grid size and uses the navigator to switch among sibling panes; resource-tree pane creation and movement must not be presented as visible splits.
